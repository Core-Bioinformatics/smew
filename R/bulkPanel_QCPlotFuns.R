...
plsda_contrib <- function(expression.matrix,
  ...
  print(head(contrib))
  contrib.plot = ggplot(head(contrib,30),aes(x=PLSDAComp,y=display_metab,fill=PLSDAComp>0))+
...
peaks_barplot <- function(sub.expression.matrix,
  ...
  log.expression.matrix$peak <- rownames(log.expression.matrix)
  ...
  p <- ggplot(melted.expression.matrix, aes(x = .data$name,
    ...
    guides(fill=guide_legend(ncol=1,nrow=length(unique(melted.expression.matrix$peak)),byrow=TRUE))
  p
}
...
peaks_boxplot <- function(sub.expression.matrix,
  ...
  p <- ggplot(melted.expression.matrix, aes(fill = .data$metadata,
                                            y = .data$value,
                                            x = .data$peak)) +
    geom_boxplot() +
    theme_minimal() +
